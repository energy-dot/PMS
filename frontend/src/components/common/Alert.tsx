...
const Alert: React.FC<AlertProps> = ({
  ...
  return (
    <div 
      className={`${bg} ${text} ${border} border p-4 rounded-md flex items-start transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'} ${className}`}
      role="alert"
    >
      ...
      {onClose && (
        <button 
          onClick={handleClose}
          className="flex-shrink-0 ml-2 inline-flex text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          aria-label="閉じる"
        >
