  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        try {
          // 案件ステータス集計データの取得
          const projectStatusResponse = await fetchWithRetry(
            `${API_BASE_URL}/reports/project_status`
          );
        ...
        try {
          // 応募状況集計データの取得
          const applicationStatusResponse = await fetchWithRetry(
            `${API_BASE_URL}/reports/application_status`
          );
        ...
        try {
          // 月別案件推移データの取得
          const monthlyTrendResponse = await fetchWithRetry(
            `${API_BASE_URL}/reports/monthly_project_trend`
          );
        ...
        try {
          // パートナー別案件数データの取得
          const partnerProjectsResponse = await fetchWithRetry(
            `${API_BASE_URL}/reports/partner_projects`
          );
                ...
                {kpiData.recentNotifications.map((notification: any) => (
                  <li key={notification.id} className="py-3">
                    <div className="flex items-start">
                      <div
                        className={`flex-shrink-0 h-2 w-2 rounded-full mt-2 ${notification.read ? 'bg-gray-300' : 'bg-blue-500'}`}
                      ></div>
                      <div className="ml-3">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {kpiData.pendingTasks &&
                      kpiData.pendingTasks.map((task: any) => (
                        <tr key={task.id}>
